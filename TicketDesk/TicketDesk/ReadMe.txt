...
TicketDesk is licensed under the terms of the Microsoft Public License (Ms-PL). Please see the License.txt file for details.
...
TicketDesk comes in three distributions:
	...
	* Pre-compiled web application:
		A ready to go binary version of TicketDesk. Just copy to aticketweb server, edit appropriate settings in web.config, and go! 
		
		-- may require additional setup for SQL server 2005 in someticketenvironments.
		
	* An updatable pre-compiled web applicaiton:
		Similar to the pre-compiled application, this version includes updatable aspx and ascx files for quick UI modifications. Just copy to a web server, edit appropriate settings in web.config, and go! 
		
		-- may require additional setup for SQL server 2005 in some environments
			
	* Source - Visual Studio 2008 Web Application Project
		Full source code for TicketDesk in a VS 2008 web application project. Allows full customization of both the code and UI. 
...
---------------------------------------------------------------------
	...
	* SQL Server Express Edition 2005 (default), or a full version of SQL Server 2005 (standard, pro, enterprise, etc.).
	 
	- NOTE: IIS 7 support was only tested with Windows Vista
	
	- If using the source distribution, requires Visual Studio 2008 or Visual Web Developer 2008. Compatible with the integrated Development Web server that ships with VS 2008 & VWD.
		
	- Email notifications (optional) require an SMTP mail server or SMTP relay (see configuration notes).
...
Source Distribution:
	...
	* The source distribtion can run directly in the built-in web server that ships with VS 2008 or VWD 2008. 
	...
	* You can deploy from VS 2008 or VWD 2008 to a web server by using the "Publish Web" feature or by manually copying the files to the web server. Don't forget to build the applicaiton in "Release" mode before you deploy it into production.
...
Web.config:
	...
	The default web.congif file is setup with default values will work on IIS 6 or IIS 7 using the SQL Express 2005 database files incldued in app_data.
...
WebAd.config:
	...
	This is a copy of web.config modified for use with Active Directory. Unlike the SQL configuration file, this file will require manual changes to run in any enviornment. Replace the web.config file with a copy of this file if you plan to use AD integration.
...
Common Web.Config settings:
	...
	* <connectionStrings>
		...
		By default TicketDesk usese two SQL Express databases in the app_data folder (as file-attached databases). 
		
		One database contains the tickets and related information. The other is the a for the SQL Security providers (membership, rles, profiles). Even when using AD security, there will still be a SQL security database required for the profile fields.
		
		If you setup your installation to use different databases you can edit the connection strings as appropriate. 
	
	* <system.net><mailSettings>
	
		If using Email notifications you must configure this section with valid settings for your the mail server that will relay email for the application. 
		
	* <machineKey>
	
		If you wish to store passwords in an encrypted format you should specify a valid machine key. This section is commented out in the default configuration since the membership provider is setup to use clear text passwords.
...
Configuration Settings with SQL Server Security:
	...
	There are sevreal common settings that you may want to review and change to configure TicketDesk for your Environment. 
	...
	* <membership>
		...
		If you wish, you can change settings for the membership provider here. A common change would be to switch to using encrypted passwords (which requires a machine key too).
		
	* <authorization>
	
		This specifies which roles are allowed to access ticketdesk. If you customize the role names in the <appsettings> section you will also have to change the roles in <authorization> as well.
...
Configuration Settings with Active Directory Security:
	...
	* <appsettings>
		...
		- ActiveDirectoryUserPassword: The passowrd for the AD user account
		...
		- HelpDeskStaffRoleName: The name of the AD group that contaions the users that will be allowed to work on tickets. These users can do everything in TicketDesk except manage user accounts or change TicketDesk configuration settings. 
	...
	* <authorization>
		...
		This specifies which AD groups are allowed to access ticketdesk. These should be the same group names as are specified in the <appsettings> section.
		
		
Security
---------------------------------------------------------------------
TicketDesk can be configured for use with SQL Server providers or Active Direcoty (SQL is the default). 
...
SQL Server Security:
	...
	TicketDesk includes a simple login and registration page as well as a simple User Management page (visible only to admin users) where the user role assignements can be changed. Users can also be deleted via the user management page.
...
AD Security: 
	...
	TicketDesk uses the WindowsTokenRole provider when configured this way, but also requires you to configure it with an AD user and password that has at least read-access to the Active Direcoty domain. This is necessary because the default role provider does not expose a way to read group membership or fetch display and email address information from AD.
	
	When using AD security, TicketDesk still requires and SQL Security database to store profile data, but user and groups will not be maintained within the application. You will have to maange the users and groups with Windows.
...
TicketDeak uses SQL Server 2005 databases and is compatible with either SQL Server Express 2005 or with full versions. 

The default 
configuration uses file attached SQL Express databases contained in the app_data folder. 

If you are using a full edition of SQL server or if you desire a permanantly attached SQL Express database, you can create the databases manually. 
