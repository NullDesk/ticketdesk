...
namespace TicketDesk.Web.Client.Controllers
{
    ...
    public class TicketActivityController : Controller
    {
        ...
        public async Task<ActionResult> ActivityPanel(int ticketId)
        {
            var ticket = await Context.Tickets.FindAsync(ticketId);
           
            return PartialView("_ActivityPanel", ticket);
        }
        ...
        public async Task<ActionResult> Assign(int ticketId, string comment, string userId, string priority)
        {
            var ticket = await Context.Tickets.FindAsync(ticketId);
            if (ModelState.IsValid)
            {
                ...
                var activityComment = ticket.TicketEvents.AddActivityEvent(Context.SecurityProvider.CurrentUserId,
                    TicketActivity.AddComment, comment,priority,User.Identity.GetUserDisplayName());
